...
namespace JonathanPolakowPROG7312Part1
{
   public partial class BookShelf : UserControl
   {
      ...
      private void PopulateBooks()
      {
         ...
         string[] Authors = { "Smith", "Jones", "Davis", "Adams", "Harris", "Murphy", "Anderson", "Walker", "Westhuizen", "Botha" };
         ...
         for (int i = 0; i < Books.Count; i++)
         {
            ...
            DeweyDecimalList.Add($"{wholeNumber:D3}.{decimalPart:D2}\n{Authors[i].Substring(0, 3).ToUpper()}");
         }

         //sort list
         DeweyDecimalList = DeweyDecimalList.OrderBy(_ => rnd.Next()).ToList();
         ...
         //create a list of the books and which nuimber was assigned
         //then sort by the number, and if two match it will sort by the authors name
         //c# OrderBy is able to handel sorting strings made up of numbers and letters
         //then save
         var mySortedList = SortedBooks.OrderBy(d => d.Value).ToList();
         SortedBooks = mySortedList.ToDictionary(x => x.Key, x => x.Value);
      }
